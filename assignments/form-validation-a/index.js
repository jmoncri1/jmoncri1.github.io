...
for (const form of document.getElementsByTagName("form")) {
    form.addEventListener("submit", function(event) {
        ...
        for(const input of inputs) {
            ...
            if (input.classList.contains("required") && input.value ==="" | !/[A-Za-z]/.test(input.value)) {
            ...
            if(input.classList.contains("numeric") && !/^[0-9]+$/.test(input.value) && /^[A-Za-z]+$/.test(input.value) && input.classList.contains("required")) {
